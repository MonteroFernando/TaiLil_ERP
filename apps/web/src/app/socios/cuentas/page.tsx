...
export default function CuentasSocios() {
  ...
  const cargar=useCallback(async()=>{const r=await apiFetch(`${apiUrl}/articulos/socios`,{credentials:"include"});if(!r.ok)return [] as Socio[];const datos:Socio[]=await r.json();setSocios(datos);return datos},[]);

...
export default function Panel() {
  ...
  useEffect(()=>{async function cargar(){const r=await apiFetch(`${apiUrl}/dashboard`);if(r.status===401){router.replace("/acceso");return}if(!r.ok){setCargando(false);return}const d=await r.json();setTarjetas(d.tarjetas);setCargando(false)}void cargar()},[router]);

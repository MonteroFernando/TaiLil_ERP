...
import { DetalleLineasComprobante } from "@/components/ComprobanteExpandible";
...
function UltimaVentaExpandible({venta}:{venta:Venta}) { return <details className="rounded-2xl border bg-white p-4 text-sm"><summary className="cursor-pointer list-none"><small className="text-[var(--texto-suave)]">Última venta · clic para ver las líneas</small><b className="block font-mono text-[var(--marca)]">{venta.numero_completo}</b></summary><div className="mt-3"><DetalleLineasComprobante datos={[{titulo:"Cliente",valor:venta.cliente_nombre},{titulo:"Estado",valor:venta.estado}]} lineas={venta.lineas.map(linea=>({id:linea.articulo_id,codigo:linea.articulo_codigo,descripcion:linea.articulo_descripcion,detalle:`Lista: ${linea.lista_nombre}`,cantidad:linea.cantidad_base,precioUnitario:linea.precio_unitario_bruto,total:linea.total_bruto}))} totales={[{titulo:"Total",valor:formatearMoneda(venta.total_bruto)},{titulo:"Saldo pendiente",valor:formatearMoneda(venta.saldo_pendiente)}]}/><div className="mt-3 flex gap-2"><button type="button" onClick={()=>window.open(`${apiUrl}/articulos/pos/ventas/${venta.id}/imprimir?formato=ticket`,"_blank")} className="rounded-lg border px-3 py-2">Ticket</button><button type="button" onClick={()=>window.open(`${apiUrl}/articulos/pos/ventas/${venta.id}/imprimir?formato=a4`,"_blank")} className="rounded-lg border px-3 py-2">A4</button></div></div></details>; }
    ...
    <main className="listas-precios-pagina flex min-h-screen flex-col overflow-x-hidden p-4 sm:p-6 xl:h-screen xl:min-h-[720px] xl:overflow-hidden">
      ...
      <form onSubmit={confirmar} className="mt-4 grid min-h-0 flex-1 gap-4 xl:grid-cols-[minmax(0,1fr)_370px]">
        ...
        <aside className="order-first space-y-3 overflow-y-auto pb-2 xl:order-none xl:min-h-0">
          ...
          {ultimaVenta&&<UltimaVentaExpandible venta={ultimaVenta}/>}
        </aside>

...
import { DetalleCompraRemoto, DetalleVentaRemoto, FilaComprobanteExpandible } from "@/components/ComprobanteExpandible";
    ...
    <section>
      ...
      {(tab === "cuentas" || tab === "conciliaciones") && <div className="mt-6">
        ...
        {tab === "cuentas" ? <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="overflow-x-auto rounded-2xl border bg-white p-4"><h2 className="font-semibold">Documentos pendientes</h2><table className="mt-4 w-full text-left text-sm"><thead><tr className="border-b text-xs uppercase text-[var(--texto-suave)]"><th className="p-2">Fecha</th><th className="p-2">Socio</th><th className="p-2">Documento</th><th className="p-2 text-right">Total</th><th className="p-2 text-right">Saldo</th><th className="p-2 text-right">A imputar</th></tr></thead><tbody>{pendientesVisibles.map((x) => <FilaPendiente key={x.id} documento={x} tipo={tipo} columnas={6}><td className="p-2">{fecha(x.fecha)}</td><td className="p-2">{x.socio_nombre}</td><td className="p-2">{x.numero_externo || `#${x.numero}`}</td><td className="p-2 text-right">{dinero(x.total)}</td><td className="p-2 text-right font-semibold">{dinero(x.saldo_pendiente)}</td><td className="p-2 text-right"><input aria-label="Importe a imputar" type="number" min="0" max={x.saldo_pendiente} step="0.01" disabled={!socioId || socioId !== x.socio_id} value={importes[x.id] ?? ""} onChange={(e) => setImportes({ ...importes, [x.id]: e.target.value })} className="w-28 rounded-lg border p-2 text-right" /></td></FilaPendiente>)}</tbody></table>{!pendientesVisibles.length && <p className="p-8 text-center text-sm text-[var(--texto-suave)]">No hay documentos pendientes.</p>}</div>
...
function FilaVenta({venta,children}:{venta:Venta;children:ReactNode}) {
  return <FilaComprobanteExpandible columnas={6} etiqueta={`comprobante ${venta.numero_completo}`} valoresOrden={[venta.fecha,venta.numero_completo,venta.socio_nombre,venta.estado,venta.total,venta.saldo_pendiente]} detalle={<DetalleVentaRemoto ventaId={venta.id}/>}>{children}</FilaComprobanteExpandible>;
}

function FilaPendiente({documento,tipo,columnas=5,children}:{documento:{id:string};tipo:"clientes"|"proveedores";columnas?:number;children:ReactNode}) {
  return <FilaComprobanteExpandible columnas={columnas} etiqueta="comprobante" detalle={tipo==="clientes"?<DetalleVentaRemoto ventaId={documento.id}/>:<DetalleCompraRemoto documentoId={documento.id}/>}>{children}</FilaComprobanteExpandible>;
}

function FilaReferencia({documentoId,tipo,children}:{documentoId:string;tipo:"clientes"|"proveedores";children:ReactNode}) {
  return <FilaComprobanteExpandible columnas={4} etiqueta="comprobante relacionado" detalle={tipo==="clientes"?<DetalleVentaRemoto ventaId={documentoId}/>:<DetalleCompraRemoto documentoId={documentoId}/>}>{children}</FilaComprobanteExpandible>;
}

...
`PosVentaCrear.destino_excedente` acepta `VUELTO` o `SALDO_FAVOR`. Un sobrepago se admite solamente hasta el efectivo ingresado y no puede combinarse con cuenta corriente. Para `VUELTO`, el servidor descuenta el excedente de los renglones de efectivo antes de crear `CobroDocumento` y `CobroMedioPago`, de modo que el control de caja refleje solo el dinero retenido. Para `SALDO_FAVOR`, exige un `cliente_id`, conserva el cobro completo e imputa a la venta solamente su total; la diferencia queda disponible según el calculo normal de anticipos. La respuesta informa `vuelto` y `saldo_favor_generado` para confirmar el resultado al cajero.

...
La pantalla reutiliza `SelectorModoImpresion`, por lo que comparte con el POS la preferencia persistida en `localStorage`. Ambos modos llaman a `window.print()`; la diferencia operativa es que Edge iniciado con `--kiosk-printing` omite el dialogo en modo directo. El corte no se emite como bytes crudos desde el navegador: lo ejecuta el driver predeterminado al finalizar trabajo o pagina, segun la capacidad de la impresora.

...
`socios.cuenta_padre_cliente_id` y `socios.cuenta_padre_proveedor_id` son claves foraneas autorreferenciadas y opcionales. El servicio valida que el padre tenga el mismo rol, además de autorreferencias y ciclos. La futura consulta de cuenta corriente podra obtener saldo individual y saldo consolidado sin mezclar clientes y proveedores. `cuenta_padre_id` queda transitoriamente por compatibilidad con migraciones anteriores.
